...
(function($){
  ...
  function InitEnv (){
    ...
    imgEnv.src = '../../dist/image/packet-icon.png';
  ...
  function evnPosition (img){
    ...
    //红包初始的位置
    var startTop = 18 + "%";
    var startLeft = 0.4 * containerWidth;//parseInt(Math.random()*(150-100)+100)
    //移动的位置
    var moveLeft = parseInt(Math.random() * containerWidth + (-(containerWidth/2)));
    var time = parseInt(Math.random() * 1400+1800);
    evnAnimation({'startLeft':startLeft, 'startTop':startTop, 'moveLeft':moveLeft,'moveTop':1000},time);
  }
  /*添加元素到主内容上*/
